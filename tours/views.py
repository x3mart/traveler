...
class TourViewSet(viewsets.ModelViewSet, TourMixin):
    ...
    def get_queryset(self):
        if self.action in ['list',]:
            ...
            qs = Tour.objects.prefetch_related(prefetchet_tour_basic, 'start_country', 'start_city', 'wallpaper').only('id', 'name', 'start_date', 'start_country', 'start_city', 'price', 'discount', 'duration', 'tour_basic', 'wallpaper').filter(is_active=True).filter(tour_basic__direct_link=False).filter(start_date__gte=datetime.today())
