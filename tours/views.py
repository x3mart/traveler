...
class StartPage(APIView):
    def get(self, request, format=None):
        ...
        types = TourType.objects.filter(Q(tours_by_basic_type__in=queryset) | Q(tours_by_additional_types__in=queryset)).annotate(tours_count=Count('tours_by_basic_type', distinct=True) + Count('tours_by_additional_types', distinct=True)).distinct()[:6]
        start_page = {
