...
def tour_review_post_save(instance, **kwargs):
    tour = instance.tour
    tour.rating = Tour.objects.filter(pk=tour.id).aggregate(avg=Avg('tour_reviews__rating'))['avg']
    tour.reviews_count = Tour.objects.filter(pk=tour.id).aggregate(count=Count('tour_reviews'))['count']
    tour.save()
